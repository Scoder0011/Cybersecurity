---
banner: "Assets/Banners/PixelCode.png"
banner_y: 0.5005
banner_x: 0.45397
banner_icon: 🪗
---
...
# Linux Fundamentals 
<<https://academy.hackthebox.com/app/module/18>>


# Linux Structure

So what is Linux >> It is an operating SYstem (OS) which helps the hardware and the software to talk to each other 

Where is it used >> It is used all over the world like in servers, phones like andriod in supercomputers,etc.

It is open source and it is not owned by anyone 
There are different versions of linux made for everyone prefences like kali linux for pentesting and ubunto for general use etc. 
And those differnet versions are called Distros

what is OS >> OS is called operating system its work is to manage all the hardware things like cpu,memory and storage so the softwares can run smoothly,Currently Linux is the OS.

## History

Linux is actually not It self an operating system OS it is a kernel only 
made by Linus Torvalds . He created this for his own use but it became one of the best OS in the world he created this when he was only 21 years old and also Created A GNU lincecing which is basicly general public license which means that it is free for everyone and anyone can use it 

There are multiple versions (distros) of the Linux some eg are ubunto,arch,kali,parrot os ,etc.

Linux OS is considered as the most secured OS in the world 


## Components In Linux System
...
So what are Distros >> these are the linux distributions based on the linux kernel 
Each distros are made for something but they all belong to the linux distros 
Each Distros have their own file system and package manager 

## Debian
Debian is an part of the linux distribution and it is mostly used and it is used almost everywhere it is used all over the world as its most stable and it used in phones , servers , web sockets etc. 
and its package manager is called apt (advance package manager )
it is used to handle all the software update and upgrades and security patches . this helps the system keep up to date 


# Introduction to Shell

so what is shell >> a linux terminal is called shell it is a text based input output terminal where u can directly talk to the OPERATING SYSTEM
it provides and interface to the user and the system to talk to each other by typing some commands in the shell and telling the OS what to do 

## Terminal Emulators

there are 2 terms GUI AND CLI Which are called graphical user interface
and the command line interface 
so what is GUI >> it is a interface which lets you talk to the server direclty from the far even when u are not present where the server is present 

so what is CLI >> cli is basicly an interface where u can run commands but u can use differnt command line interface to send instructions to the same server 
...
what is SSH >> ssh is called secure shell it is an protocol that allows the client to access and execute commands or action on remote computers.
it is mostly prefred by all the admins and it permanent installed software and it is used in all kind of OS 


# Navigation

its basicly using your whole OS with just commands lines each command line does different work like pwd shows u current directory and ls list all the files and so on
...
To create the files with command we use Scoder01@htb[/htb]$ touch <name>
and to create the directory we use Scoder01@htb[/htb]$ mkdir <name>

We can look at the whole structure after creating the parent directories with the tool `tree`.

With the command `mv`, we can move and also rename files and directories. The syntax for this looks like this: Scoder01@htb[/htb]$ mv <file/directory> <renamed file/directory>

To Edit file we can use test editors some of them there is one called nano its a editor where we can edit texts 

To View the content of any file we can use cat and the filename cat text.txt it will show all the content of that file inside it in the shell 


## Find Files and Directories

to gain access to all the driectories or find all the files we can use cetrain commands or tools already present one of the tools are called (which)
`which`. This tool returns the path to the file or link that should be executed. This allows us to determine if specific programs, like cURL, netcat, wget, python, gcc, are available on the operating system. Let us use it to search for Python in our interactive instance.
Scoder01@htb[/htb]$ which python

There is another tool called `find`. Besides the function to find files and folders, this tool also contains the function to filter the results. We can use filter parameters like the size of the file or the date. We can also specify if we only search for files or folders.
Scoder01@htb[/htb]$ find <location> <options>
...
THEN There is another tool called localte its basicly `locate` offers us a quicker way to search through the system. In contrast to the `find` command, `locate` works with a local database that contains all information about existing files and folders. We can update this database with the following command.

Scoder01@htb[/htb]$ sudo updatedb

If we now search for all files with the "`.conf`" extension, you will find that this search produces results much faster than using `find`.
`Scoder01@htb[/htb]$ locate *.conf /etc/GeoIP.conf /etc/NetworkManager/NetworkManager.conf /etc/UPower/UPower.conf /etc/adduser.conf <SNIP>`


File Descriptors and Redirections

an Fd is an refrence in OS to the kernel level that the kernel keep tracks of each and every file that is written givin input  or output to it 

By default, the first three file descriptors in Linux are:

1. Data Stream for Input
    - `STDIN – 0` (data standard input)
2. Data Stream for Output
    - `STDOUT – 1`* (data standard output)
3. Data Stream for Output that relates to an error occurring.
    - `STDERR – 2` (data error handling)



# Filter Contents

there are 2 tools for filtering out the content of the file `more` and `less`. These are known as pagers, and they allow you to view the contents of a file interactively, one screen at a time. While both tools serve a similar purpose, they have some differences in functionality, which we'll touch on later.
...
Scoder01@htb[/htb]$ head /etc/passwd
...
Scoder01@htb[/htb]$ tail /etc/passwd
...
Scoder01@htb[/htb]$ cat /etc/passwd | sort
...
Scoder01@htb[/htb]$ cat /etc/passwd | grep "/bin/bash"


## Cut
Specific results with different characters may be separated as delimiters. Here it is handy to know how to remove specific delimiters and show the words on a line in a specified position. One of the tools that can be used for this is `cut`. Therefore we use the option "`-d`" and set the delimiter to the colon character (`:`) and define with the option "`-f`" the position in the line we want to output.
Scoder01@htb[/htb]$ cat /etc/passwd | grep -v "false\|nologin" | cut -d":" -f1
...
Scoder01@htb[/htb]$ cat /etc/passwd | grep -v "false\|nologin" | tr ":" " "
...
Scoder01@htb[/htb]$ cat /etc/passwd | grep -v "false\|nologin" | tr ":" " " | column -t
...
Scoder01@htb[/htb]$ cat /etc/passwd | grep -v "false\|nologin" | tr ":" " " | awk '{print $1, $NF}'
...
Scoder01@htb[/htb]$ cat /etc/passwd | grep -v "false\|nologin" | tr ":" " " | awk '{print $1, $NF}' | sed 's/bin/HTB/g'
...
Scoder01@htb[/htb]$ cat /etc/passwd | grep -v "false\|nologin" | tr ":" " " | awk '{print $1, $NF}' | wc -l
...
 both the owner and the group, determining what actions—like reading, writing, or executing—are allowed.
 `execute` permissions on a directory is like having permission to walk through a hallway to access the rooms inside
`execute` permissions are necessary to traverse a directory, no matter the user's level of access. Also, `execute` permissions on a directory do not allow a user to execute or modify any files or contents within the directory, only to traverse and access the content of the directory.
...
 These bits function like temporary access passes, enabling users to run certain programs with the privileges of another user or group
...
We can use many different package management systems that cover different types of files like ".deb", ".rpm", and others
...
Scoder01@htb[/htb]$ apt-cache search impacket
Scoder01@htb[/htb]$ apt-cache show impacket-scripts
Scoder01@htb[/htb]$ apt list --installed
Scoder01@htb[/htb]$ sudo apt install impacket-scripts -y

## Git
we can clone repos using git clone and use it by our own or change codes inti 

## DPKG

We can also download the programs and tools from the repositories separately.
Scoder01@htb[/htb]$ wget
...
. All processes in a Linux system are assigned a `PID` and can be viewed under the `/proc/` directory, which contains information about each process. Processes may also have a Parent Process ID (`PPID`), indicating that they were started by another process (the parent), making them child processes.
...
Scoder01@htb[/htb]$ vim tmpfile
Scoder01@htb[/htb]$ jobs
...
- Pipes (`|`)
Scoder01@htb[/htb]$ echo '1'; echo '2'; echo '3'
Scoder01@htb[/htb]$ echo '1'; ls MISSING_FILE; echo '3'
...
by understanding the task scheduling we can prevent backdoors and understand it 
...
#### Install OpenSSH
Scoder01@htb[/htb]$ sudo apt install openssh-server -y
#### Server Status
Scoder01@htb[/htb]$ systemctl status ssh
Scoder01@htb[/htb]$ ssh cry0l1t3@10.129.17.122
...
#### Install NFS
Scoder01@htb[/htb]$ sudo apt install nfs-kernel-server -y

#### Server Status
Scoder01@htb[/htb]$ systemctl status nfs-kernel-server
...
Scoder01@htb[/htb]$ sudo apt install apache2 -y
...
Scoder01@htb[/htb]$ sudo apt install python3 -y
Scoder01@htb[/htb]$ python3 -m http.server
...
Scoder01@htb[/htb]$ python3 -m http.server --directory /home/cry0l1t3/target_files

Scoder01@htb[/htb]$ python3 -m http.server 443
...
Organizations primarily utilize VPNs to grant their employees secure access to the internal network without requiring them to be on-site
...
#### Install OpenVPN
Scoder01@htb[/htb]$ sudo apt install openvpn -y
...
#### Connect to VPN
Scoder01@htb[/htb]$ sudo openvpn --config internal.ovpn


# Working with Web Services

Another crucial element in web development is the communication between browsers and web servers. Setting up a web server on a Linux operating system can be done in several ways, with popular options including Nginx, IIS, and Apache. Among these, Apache is one of the most widely used web servers
...
Scoder01@htb[/htb]$ wget http://localhost
...
The `inode` table is a collection of these inodes, essentially acting as a database that the Linux kernel uses to track every file and directory on the system.
...
 Linux involves dividing the physical storage space into separate, logical sections. Each partition can then be formatted with a specific file system, such as ext4, NTFS, or FAT32, and can be mounted as a separate file system. The most common partitioning tool on Linux is also `fdisk`, `gpart`, and `GParted`.

#### Fdisk
Scoder01@htb[/htb]$ sudo fdisk -l
...
The `mount` command is commonly used to manually mount file systems on Linux. However, if you want certain file systems or partitions to be automatically mounted when the system boots, you can define them in the `/etc/fstab` file.

#### Mounted File systems at Boot
Scoder01@htb[/htb]$ cat /etc/fstab

To view the currently mounted file systems, we can use the `mount` command without any arguments.

#### List Mounted Drives
Scoder01@htb[/htb]$ mount

To mount a file system, we can use the `mount` command followed by the device name and the mount point.

#### Mount a USB drive
Scoder01@htb[/htb]$ sudo mount /dev/sdb1 /mnt/usb 
Scoder01@htb[/htb]$ cd /mnt/usb && ls -l
...
#### Unmount
Scoder01@htb[/htb]$ sudo umount /mnt/usb
...
The size of the `swap space` is not fixed and depends on your system's physical memory and intended usage.
...
#### Docker Build
Scoder01@htb[/htb]$ docker build -t FS_docker .
...
#### Docker Run - Syntax
Scoder01@htb[/htb]$ docker run -p <host port>:<docker port> -d <docker container name>

#### Docker Run
Scoder01@htb[/htb]$ docker run -p 8022:22 -p 8080:80 -d FS_docker
...
#### Install LXC
Scoder01@htb[/htb]$ sudo apt install lxc -y

#### Creating an LXC Container
Scoder01@htb[/htb]$ sudo lxc-create -n linuxcontainer -t ubuntu
...
Let us limit the resources to the container. In order to configure `cgroups` for LXC and limit the CPU and memory, a container can create a new configuration file in the `/usr/share/lxc/config/<container name>.conf` directory with the name of our container
Scoder01@htb[/htb]$ sudo vim /usr/share/lxc/config/linuxcontainer.conf
...
Scoder01@htb[/htb]$ sudo ifconfig eth0 up # 
OR
Scoder01@htb[/htb]$ sudo ip link set eth0 up

#### Assign IP Address to an Interface
Scoder01@htb[/htb]$ sudo ifconfig eth0 192.168.1.2

#### Assign a Netmask to an Interface
Scoder01@htb[/htb]$ sudo ifconfig eth0 netmask 255.255.255.0
...
Scoder01@htb[/htb]$ sudo route add default gw 192.168.1.1 eth0

#### Editing DNS Settings

Scoder01@htb[/htb]$ sudo vim /etc/resolv.conf

After completing the necessary modifications to the network configuration, it is essential to ensure that these changes are saved to persist across reboots. This can be achieved by editing the `/etc/network/interfaces`


It’s important to note that changes made directly to the `/etc/resolv.conf` file are not persistent across reboots or network configuration changes. This is because the file may be automatically overwritten by network management services like `NetworkManager` or `systemd-resolved`.

#### Editing Interfaces
Scoder01@htb[/htb]$ sudo vim /etc/network/interfaces

#### Restart Networking Service
Scoder01@htb[/htb]$ sudo systemctl restart networking
...
#### Ping
Scoder01@htb[/htb]$ ping <remote_host>

another tool is the `traceroute`, which traces the route packets take to reach a remote host. It sends packets with increasing Time-to-Live (TTL) values to a remote host and displays the IP addresses of the devices that the packets pass through

Scoder01@htb[/htb]$ traceroute www.inlanefreight.com
...
On the other hand, `TCP wrappers` focus on controlling access to network services based on client IP addresses, which makes it simpler but limited to network-level access control.
...
Scoder01@htb[/htb]$ ssh -L 5901:127.0.0.1:5901 -N -f -l htb-student 10.129.14.130


#### Connecting to the VNC Server
Scoder01@htb[/htb]$ xtightvncviewer localhost:5901
...
Scoder01@htb[/htb]$ apt update && apt dist-upgrade
...
#### /etc/hosts.allow
Scoder01@htb[/htb]$ cat /etc/hosts.allow

#### /etc/hosts.deny
Scoder01@htb[/htb]$ cat /etc/hosts.deny
...
## Iptables
The iptables utility provides a flexible set of rules for filtering network traffic based on various criteria such as source and destination IP addresses, port numbers, protocols, and more. There also exist other solutions like nftables, ufw, and firewalld. `Nftables` provides a more modern syntax and improved performance over iptables. However, the syntax of nftables rules is not compatible with iptables, so migration to nftables requires some effort. `UFW` stands for “Uncomplicated Firewall” and provides a simple and user-friendly interface for configuring firewall rules. UFW is built on top of the iptables framework like nftables and provides an easier way to manage firewall rules.
...
Scoder01@htb[/htb]$ sudo iptables -A INPUT -p tcp --dport 22 -j ACCEPT
...
Scoder01@htb[/htb]$ sudo iptables -A INPUT -p tcp -m tcp --dport 80 -j ACCEPT
...
# Solaris
Solaris is a Unix-based operating system developed by Sun Microsystems (later acquired by Oracle Corporation) in the 1990s. It is known for its robustness, scalability, and support for high-end hardware and software systems. Solaris is widely used in enterprise environments for mission-critical applications, such as database management, cloud computing, and virtualization.


The goal of Solaris is to provide a highly stable, secure, and scalable platform for enterprise computing. It has built-in features for high availability, fault tolerance, and system management, making it ideal for mission-critical applications. It is widely used in the banking, finance, and government sectors, where security, reliability, and performance are paramount. I

## Linux Distributions vs Solaris
Solaris and Linux distributions are two types of operating systems that differ significantly.
